...
\begin{frame}
	...
	\tikzstyle{every node} = [rectangle, fill=color1!40!color2, align=center, font=\scriptsize, draw=black, ultra thin]
	\node (input_data) at (0,0) {Input\\Data};
	\node (train_data) at (1,1.5) {Training\\Data};
	\node (test_data) at (1,-1.5) {Test\\Data};
	\node[fill=emerald, text=white] (feat_sel) at (3.5,1.5) {Feature\\Selection};
	\node[fill=darkorange, text=white] (mRMR) at (3.5,3) {mRMR};
	...
	\node (class_label) at (11,-3) {Class Label};
