...
\begin{frame}
	...
	$$\max\frac{Dep(S,c)}{Red(S)}$$
	dove ricordiamo, in caso di variabili discrete
	\begin{itemize}
		\item $Dep(S,c)= \frac{1}{|S|} \sum\limits_{x_i \in S} F(x_i;c)$
		\item $Red(S)= \frac{1}{|S|^2} \sum\limits_{x_i,x_j \in S} |c (x_i;x_j)|$
	\end{itemize}
...
\begin{frame}
	\frametitle{Benefici}
	\begin{itemize}
		\item With the same number of features, we expect the MRMR
		feature set to be more representative of the target phenotypes, therefore leading to better generalization property
		\item Equivalently, we can use a smaller MRMR feature set to effectively cover the same space as a larger conventional feature set does
	\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
